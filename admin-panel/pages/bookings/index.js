...
export default function ViewBookings() {
  ...
  const handleMarkAsRead = async (id, e) => {
    ...
    try {
      setBookings((prev) =>
        prev.map((b) => (b._id === id ? { ...b, isRead: true } : b)),
      );
      await axios.put(
        `${API_BASE}/bookings/${id}/read`,
        {},
        {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_BASE}/bookings/vendor?page=1&limit=100`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleCancel = async () => {
    ...
    try {
      await axios.put(
        `${API_BASE}/bookings/${cancelModalId}/cancel`,
        { reason: cancelReason },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const filteredBookings = Array.isArray(bookings)
    ? bookings.filter((b) => {
        ...
        const matchesSearch =
          ...
          (b.listingId?.title &&
            b.listingId.title
              ...
              .includes(searchTerm.toLowerCase())) ||
          (b.paymentMethod &&
            b.paymentMethod.toLowerCase().includes(searchTerm.toLowerCase())) ||
          (b.status &&
        ...
        const matchesType =
          typeFilter === "all" ||
          (b.listingId?.type &&
            b.listingId.type.toLowerCase() === typeFilter.toLowerCase());

        // Payment method filter check
        const matchesPayment =
          paymentFilter === "all" ||
          (b.paymentMethod &&
            b.paymentMethod.toLowerCase() === paymentFilter.toLowerCase()) ||
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {loading
                ? [...Array(6)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      ...
                      <td className="p-4 pr-6">
                        <div className="h-8 bg-slate-200 dark:bg-slate-700 rounded-lg w-40 ml-auto"></div>
                      </td>
                    </tr>
                  ))
                : paginatedBookings.map((b) => (
                    <tr
                      key={b._id}
                      className={`hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors ${!b.isRead ? "bg-emerald-50/60 dark:bg-emerald-950/25 border-l-4 border-emerald-500" : ""}`}
                    >
                      <td className="p-4 pl-6">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500 dark:text-slate-400">
                              {isGuestEmail(b.userId?.email) ? (
                                b.details?.customerEmail ? (
                                  <span title="Widget booking contact email">
                                    {b.details.customerEmail}
                                  </span>
                                ) : (
                                  <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-300">
                                    Widget Guest
                                  </span>
                                )
                              ) : (
