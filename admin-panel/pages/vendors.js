...
export default function Vendors() {
  ...
  const filteredVendors = Array.isArray(vendors)
    ? vendors.filter((v) => {
        ...
        // const matchesPhone =
        //   !phoneSearch || (v.phone && v.phone.includes(phoneSearch));
        ...
        return matchesSearch && matchesStatus && matchesDate; // && matchesPhone;
      })
    : [];

  const ITEMS_PER_PAGE = 8;
  const localTotalPages =
    Math.ceil(filteredVendors.length / ITEMS_PER_PAGE) || 1;
    ...
    switch (action) {
      case "approve":
        return {
          ...
          description:
            "Are you sure you want to approve this vendor? Their account will be activated and given access to listing tools.",
      ...
      case "activate":
        return {
          ...
          description:
            "Are you sure you want to reactivate this vendor account?",
          confirmBtnClass: "bg-emerald-600 hover:bg-emerald-700 text-white",
      ...
      case "suspend":
        return {
          ...
          description:
            "Are you sure you want to suspend this vendor? They will not be able to log in or process bookings until reactivated.",
                ...
                : paginatedVendors.map((vendor) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${vendor.status === "active" ? "bg-emerald-500" : vendor.status === "suspended" ? "bg-amber-500" : vendor.status === "inactive" ? "bg-slate-400" : "bg-blue-500"}`}
                          ></span>
                          {vendor.status || "pending"}
                        </span>
                      </td>
                      <td className="p-4 pr-6 text-right">
                        <div className="flex items-center gap-2 justify-end">
                          ...
                          {vendor.status === "pending" && (
                            <button
                              onClick={() =>
                                triggerActionPrompt(vendor, "approve")
                              }
                          ...
                          {(vendor.status === "active" || !vendor.status) && (
                            <button
                              onClick={() =>
                                triggerActionPrompt(vendor, "suspend")
                              }
                          ...
                          {(vendor.status === "suspended" ||
                            ...
                            <button
                              onClick={() =>
                                triggerActionPrompt(vendor, "activate")
                              }
                          ...
                          {vendor.status !== "inactive" && (
                            <button
                              onClick={() =>
                                triggerActionPrompt(vendor, "inactivate")
                              }
