    ...
    const fetchVendors = async () => {
        ...
        try {
            ...
            const res = await axios.get(
                `${API_BASE}/admin/vendors?page=${page}&limit=5`,
                {
            ...
            <div className="card p-0! overflow-hidden mb-6">
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse min-w-200">
                        <thead>
                            <tr className="border-b border-gray-500 dark:border-slate-700 text-left text-secondary dark:text-slate-400 text-xs uppercase">
                        ...
                        <tbody>
                            {loading
                                ? [...Array(5)].map((_, i) => (
                                    <tr
                                        key={i}
                                        className="animate-pulse border-b border-border dark:border-slate-700"
                                    >
                                        <td className="p-4">
                                ...
                                : filteredVendors.map((vendor) => (
                                    <tr
                                        key={vendor._id}
                                        className="border-b border-border dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                                    >
                                        <td className="p-4">
                                        ...
                                        <td className="p-4 text-sm text-secondary dark:text-slate-400">
                                            {vendor.createdAt
                                            ...
                                            <div className="flex gap-2 justify-end">
                                                {vendor.status === "pending" && (
                                                    <button
                                                        ...
                                                        className="btn btn-accent px-3 py-1 text-xs font-bold"
                                                    >
                                                        Approve
                                                    </button>
                                                )}
